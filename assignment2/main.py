...
texts = {col: ax.text(0, 0, '', color='black', fontsize=10, ha='left', va='center', visible=False) for i, col in enumerate(data.columns[1:])}
...
def update(frame):
    if frame < len(data):
        ...
        ax.set_title(f"Progress as of {data['Date'].iloc[frame].strftime('%Y-%m-%d')}", fontsize=14, fontweight='bold')
        for text in texts.values():
            text.set_visible(False)
    else:
        final_frame = len(data) - 1
        for col, line in lines.items(): #TODO show values at the end of each strategy
            line.set_data(data['Date'][:final_frame + 1], data[col][:final_frame + 1])
            y_value = data[col].iloc[-1] * 100
            x_value = data['Date'].iloc[-1]
            texts[col].set_position((date2num(x_value), y_value))
